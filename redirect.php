...
function getGeoLocation($ip) {
    // API ip-api.com (free, no key, simple JSON)
...
// Vérifier mot de passe si besoin
if ($url_data['password']) {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!password_verify($pass, $url_data['password'])) {
            ...
            // mot de passe correct : continuer vers redirection
            $password_ok = true;
        }
    }
    if (empty($password_ok)) {
        // Afficher formulaire de mot de passe
        ?>
        <!DOCTYPE html>
        <html lang="fr">
